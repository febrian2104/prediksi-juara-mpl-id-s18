...
def test_official_season18_snapshot_is_complete_and_time_guarded() -> None:
    ...
    assert len(teams) == 9
    assert len(rosters) == 86
    active_rosters = rosters.loc[rosters["valid_to"].isna()]
    assert len(active_rosters) == 84
    assert active_rosters["member_type"].eq("player").sum() == 64
...
def test_week_six_results_preserve_prior_results_and_hide_future_outcomes() -> None:
    ...
    assert week_six["official_match_id"].tolist() == list(range(1075, 1084))
    assert week_six["status"].eq("completed").all()
    assert list(
        week_six[
    ...
    pd.testing.assert_frame_equal(
        schedule.loc[schedule["official_match_id"].le(1074), unchanged_columns].reset_index(
            drop=True
        ),
        archived.loc[archived["official_match_id"].le(1074), unchanged_columns].reset_index(
            drop=True
        ),
    )
    future = schedule.loc[schedule["week"].gt(6)]
...
def test_roster_update_preserves_history_and_dates_new_observations() -> None:
    ...
    _, archived, _ = _load_season18(SEPTEMBER_14_DIR)
    ...
    ended = rosters.loc[rosters["valid_to"].eq("2026-09-21")]

    assert added == {
        ("EVOS", "EgaTzy", "roamer"),
        ("ONIC", "Killuaa", "mid_lane"),
        ("RRQ", "Faviannn", "jungler"),
    }
    assert ended.empty
    added_rows = active.loc[active["valid_from"].eq("2026-09-21")]
    assert len(added_rows) == 3
    assert added_rows["observed_at"].eq("2026-09-21").all()
